...
def build_image_prompt(headline: str) -> str:
    ...
    По требованию картинки должны быть максимально фотореалистичными — на
    практике диффузионные модели (бесплатные, без ключа) выдают убедительное
    фото только на простых кадрах: крупный план детали (мяч, бутса, поле) или
    атмосферный план стадиона. Как только в кадре появляются лица крупным
    планом, командная форма с номерами или несколько одинаковых предметов
    (два мяча вместо одного) — вылезают типичные огрехи ИИ-картинок
    (задвоенные объекты, "варёные" пальцы, нечитаемый текст-кашица на форме),
    и результат сразу выглядит нарисованным. Поэтому промпт намеренно не
    просит портретный кадр конкретного игрока, а держит фокус на детали или
    атмосфере — это осознанный компромисс: получаем "фотографично", а не
    "иллюстрацию именно этого события".
    """
    return (
        f"Профессиональная спортивная фотография, тема: {headline}. "
        f"Крупный план детали (мяч, бутсы, поле, свет) или атмосферный кадр "
        f"стадиона — без лиц крупным планом и без командной формы с номерами. "
        f"Стиль: {IMAGE_STYLE}. "
